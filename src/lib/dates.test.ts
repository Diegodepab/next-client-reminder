...
describe('calculateNextServiceDate', () => {
    ...
    it('handles month-end overflow (Jan 31 + 1 month)', () => {
        ...
        // JS Date rolls Jan 31 + 1 month = Mar 2 (because Feb has 29 days in 2024)
...
describe('calculateReminderDate', () => {
    it('returns exactly 7 days before the input date', () => {
        const nextService = new Date(Date.UTC(2024, 0, 15)); // Jan 15 UTC
...
describe('shouldNotify', () => {
    it('returns true when today equals the reminder date', () => {
        const reminder = new Date(Date.UTC(2024, 4, 10)); // May 10 midnight UTC
        const today = new Date(Date.UTC(2024, 4, 10, 9)); // May 10 9 AM UTC
